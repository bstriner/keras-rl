    ...
    def call(self, x, mask=None):
        ...
        if self.mode == 'full':
            ...
            if K.backend() == 'theano':
                ...
                indices = tf.constant(np.vstack(np.expand_dims(i, 0) for i in np.tril_indices(self.nb_actions)))
                shape = tf.constant([self.nb_actions, self.nb_actions])

                def fn(_, x_tri):
                    # Scatter into a lower triangular matrix
                    x_ = tf.scatter_nd(indices=indices, updates=x_tri, shape=shape)
                    x_ = tf.dot(x_, tf.transpose(x_)) + (tf.eye(self.nb_actions)*K.epsilon())
                    return [x_]
